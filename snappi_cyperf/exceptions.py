...
class Snappil47Exception(Exception):
    ...
    def args(self):
        return (
            self._status_code,
            ([self._message] if not isinstance(self._message, list) else self._message),
            self._url,
        )
    ...
    def process_exception(self):
        ...
        if isinstance(self._args[0].args, tuple) and len(self._args[0].args) == 1:
            ...
            if isinstance(
                self._args[0],
                (NameError, TypeError, ValueError, KeyError, AttributeError),
            ):
                self._status_code = 400
                self._message = self._args[0].args
                return (self._status_code, self._message)
            if isinstance(self._args[0], (ImportError, RuntimeError)):
                self._status_code = 400
                self._message = self._args[0]
                return (self._status_code, self._message)
            else:
